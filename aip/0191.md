...
particular recommendations are irrelevant.
...
itself. An API with a small number of discrete services (Pub/Sub's `Publisher`
and `Subscriber` is a good example) **may** have a separate entry file per
service.
...
descriptive filename does matter.
