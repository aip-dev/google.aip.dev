...
example above), and a Docker entrypoint **must** route them to the appropriate
`--{lang}_gapic_opt` arguments. However, generators **should not** require
arguments, and this should be a last resort if they are unavoidable.
...
**TL;DR:** ach language probably wants `language:x.y-alpine` or
...
the API protos on his or her host machine, and will ultimately need the output
to go to said host machine, Docker images **should** use two mount points. This
