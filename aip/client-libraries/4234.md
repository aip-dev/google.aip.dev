...
Client library generators **must not** generate a mixin method on a host
...
same RPCs already defined in the `LibraryService` on its client with mixin
methods. In this case, the generator **may** log a warning indicating that a
collision was avoided, or silently skip the mixin methods in question.
