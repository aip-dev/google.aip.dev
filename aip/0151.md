...
- The method **must** include a `google.longrunning.operation_info` annotation
  ...
  - The metadata type **should not** be `google.protobuf.Empty`, as this
    prevents metadata from being added later. Instead, define an empty message
    for the RPC and use that.
...
resource **should** be included in List and Get calls; however, the resource
**should** indicate that it is not usable, generally with a [state
enum][aip-216].
