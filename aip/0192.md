...
prefer people who are non-controversial and no longer alive.

### Style

Comments **should** generally be in gramatically correct American English.
However, the first sentence of each comment **should** omit the subject:
...
comments are necessarily perfunctory because there is nothing else to be said.
Before jumping to that conclusion, however, consider whether some of the
following questions are relevant:

- What is it?
- What does it do if it succeeds? What does it do if it fails? What can cause
  it to fail, and how?
- Is it idempotent?
...
- Is it always present? (For example: "Container for voting information.
  Present only when voting information is recorded.")
...
unsuitable for docstrings in client libraries.
