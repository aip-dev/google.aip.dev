...
For methods that operate on a resource, the completion of those operations
(either successful or with an error) **must** mean that the state of the
resource's existence and all user-settable values have reached a steady-state.
...
- A get request for a resource immediately after the successful completion of a
  create method **must** return the resource.
- A get request for a resource immediately following an update **must** return
  a resource with the final values from the update request.
