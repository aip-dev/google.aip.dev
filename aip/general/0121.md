...
- Each resource **must** have a unique [resource name][] by which it can be
  identified and referenced.
- The resource API must support [Get][].
- The resource API must support [List][], with the exception of [singleton
  resources][].
- Resources must operate over a stateless protocol: any individual request is
  independent of any other requests made by the caller.
