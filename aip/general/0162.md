...
- The resource revision **may** contain a repeated field `tags`, which would
  contain a list of resource IDs that the revision is also known by (e.g. `current`)
...
APIs **may** use any of these strategies.

### Resource names for revisions

When referring to specific revision of a resource, the subcollection name
**must** named `revisions`. Resource revisions have names with the format
