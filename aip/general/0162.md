...
The resource revision type **must** inherit from the parent resource and has a
field being the parent resource type.
...
APIs **may** use any of these strategies. If a strategy is used, they **must**
...
### Server specified IDs

Services **may** reserve specific IDs to be server-specified (e.g. `current`,
...
Rather than create a revision via the standard method, it may be more convenient
to create a revision snapshotting the current resource configuration.

APIs **may** provide a mechanism for users to use the existing revisions or
current resource state to create a new revision with a custom method "tag":
...
message TagBookRevisionRequest {
  ...
  // If unset, the current resource configuration will used to create the
  // revision.
  //
  // If set, the configuration in the specified revision will be used. This is
  // helpful when one wants to set an alias for the resource, such as
  // `current`.
...
- The request message **must** have a `name` field:
  ...
  - The field **must** identify the [resource type][aip-123] that it
    references.
- The request message **must** have a `revision_id` field:
  - The field **must** be [annotated as optional][aip-203].
- The request message **must** have a `source_revision_id` field:
  - The field **must** be [annotated as optional][aip-203].
- If the user calls the tag method with an existing `revision_id`, the request
  **must** succeed and the revision will be updated with the configuration for
  the specified resource. This allows users to write code against specific tags
  (e.g. `published`) and the revision can change in the background with no code
  change.
...
If a revision implements standard methods, they **must** implement the standard
methods ([AIP-131][], [AIP-132][], [AIP-133][], [AIP-134][], [AIP-135][]).
...
In 2023-09, revisions are abstracted as nested resource. Prior to this,
...
a revision shares the same method with get resource.

It's primary advantage was allowing a resource reference to seamlessly refer to
a resource, or it's revision.
...
In th previous design, revisions had a separate identifer for a revision known
...
- **2023-08-07**: AIP was updated to be a sub-collection.
