...
**Note:** When supplying a user-specified ID, while the ID is set on
the request message for RPCs, for REST APIs, the `{resource}_id` field
could be included as a query parameter when making the the request.
