...
(#changing-resource-names) as an example). APIs **may** forward declare the
upper bound of expected size and/or the limit (if accepted as input) in field
documentation. APIs **may** pad out values with filler characters if reserving a
...
limit (if accepted as input) in field documentation.
