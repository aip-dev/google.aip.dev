...
+ `reason string`
  ...
  + In contrast, using either of the following reasons is not
    ...
    using either of the following reaons breaches the required
...
+ `metadata map`
  + Additional structured details about this error, which **should**
    ...
    steps.  Keys **should** match `/[a-zA-Z0-9-_]/` and be limited to 64
    characters in length. When identifying the current value of an
    exceeded limit, the units **should** be contained in the key, not
    the value.
  + *Example*:
   ```
   "vmType": "e2-medium",
