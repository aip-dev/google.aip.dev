...
+ For pre-existing (brownfield) APIs, the information in 
 ...
 in text for the `message` string could cause breakages. For more
 information, see [Changing Error Messages](#changing-error-messages).
...
+ `reason string`
  ...
  + In contrast, using either of the following reasons is not recommended:
    `THE_BOOK_YOU_WANT_IS_NOT_AVAILABLE`, `ERROR`. And, using either of
    the following reaons breaches the required formatting and is not
    allowed: `librariesAreGreat`, `noBooks`.
+ `domain string`
