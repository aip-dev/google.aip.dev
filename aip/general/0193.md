...
[`google.rpc.Status`][Status] message when an API error occurs. For each
error, the service **must** populate the `message` string in
...
- `reason string`
  ...
  - Distill your error message into its simplest form. For example, if
    ...
    following text examples in UPPER_SNAKE_CASE: UNAVAILABLE, NO_STOCK, CHECKED_OUT, AVAILABILITY_ERROR
...
present the service's default locale; for example, "en-US".
...
[help-example]: https://cloud.google.com/PRODUCT/docs/ERROR-REASON
