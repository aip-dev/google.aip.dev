...
AIPs **must** contain API guidance that provide a clear benefit to authors of
one or more types of clients or their authors, including but not limited to:
...
- Reccomendation tools which provide guidance on which APIs are useful for
...
AIP guidance **must not** be a signifcant detriment to a client's usability.
