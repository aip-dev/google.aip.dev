...
AIPs **must** contain API guidance that provide a clear benefit to one or more
clients, including but not limited to:

- Command line interfaces.
- [Infrastructure as Code][] tools.
- SDKs to interact with an API from a programming language.
- Visual User Interfaces.
- Users.

AIP guidance **must not** contain guidance that significantly detriments a
client.
...
APIs via a variety of methods, depending on their use case:
...
- SDKs for data-plane operations.

API guidance must in turn consider each of these different clients.
