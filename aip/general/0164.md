...
Resources that support soft delete **may** provide an `Expunge` custom method to allow users to trigger immediate permanent deletion of a ready or soft-deleted resource. This method can operate on resources that are currently in a ready or soft-deleted state (e.g., `delete_time` is set).
