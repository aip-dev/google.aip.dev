...
- Scripts that operates on a JSON representation of a resource after reading it
  from an API
- Tools, such as an [State-driven][] client
...
State-driven Clients describes a category of clients that
...
desired state. To determine if a set of updates was successful a state-driven
client compares server side resource attributes with client defined values.
The comparison feature ensures accuracy of a creation or an update but it
requires services to treat the client set fields as read-only and diligently
...
[State-driven][] clients entering an infinite loop to correct the change.

[State-driven]: #state-driven-clients
