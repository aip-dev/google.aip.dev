...
- One Platform APIs, which support JSON primitive types like `string` and the
...
Only certain Schema.org entity types will make sense to map to a given common
type. In order to support BII arguments with types that cannot be mapped to an
...
One Platform APIs which support BII argument mapping **should** support at
