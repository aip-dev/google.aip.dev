...
- Enum values **may** be freely added to enums in request messages. For enums
  in response messages and resources, be aware that it is possible that user
  code does not handle new values gracefully. In this situation, enums which
  are expected to receive new values **should** document this, and while enum
  values still **may** be added in this situation, appropriate caution
  **should** be used.
