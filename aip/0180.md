...
**Important:** It is not always salient whether a change is compatible or not.
...
3. Semantic compatibility: Code written against a previous version **must**
   continue to receive the intended and expected results.
...
- Enums **may** be freely added to request messages. For response messages and
  resources, be aware that it is possible that user code does not handle new
  values gracefully. In this situation, enums which are expected to receive new
  values **should** document this, and while enum values still **may** be added
  in this situation, appropriate caution **should** be used.
...
add the new component, but **must not** remove the existing one.
...
**Note:** An exception to this rule is that `int32` values **may** be changed
to `int64`.
...
visible behavior or semantics, as such changes will be seen as breaking by
users.
