...
infrastructure and tooling to provide a better user experience around times.
...
be named with the `{verb}_time` form. Fields **should not** be named using the
past tense (such as `created_time` or `last_updated_time`).
...
- For integers, include the meaning (examples: `time`, `duration`, `delay`,
  ...
  `millis`, `micros`, `nanos`) as a final suffix. Example: `send_time_millis`
