...
[Resource-oriented design][] honors this pattern through the `List` method.
These RPCs accept the parent collection (and potentially some other
...
- The `page_size` and `page_token` fields, which support pagination, **must**
  be included on all list request messages. For more information, see
  [AIP-158][].
...
- The message **may** include a `int32 total_size` (or `int64 total_size`) field
  with the number of items in the collection.
...
- Redundant space characters in the syntax are insignificant. `"foo, bar desc"`,
  `" foo , bar desc "`, and `"foo,bar desc"` are all equivalent.
...
`bool show_deleted` field in the list request that, if set, will cause deleted
