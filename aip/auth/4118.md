  ...
  - Locate the workload certificate and private key files using the above
    ...
    passing them to the TLS library.
      - Occasional mismatches may happen, since during certificate rotation the
        ...
        The library **should** wait for 5 seconds between retries. In total,
        the library **should** attempt loading these files up to 4 times,
        including up to three retries if the first load attempt failed.
