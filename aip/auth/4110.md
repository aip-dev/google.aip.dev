...
1. **Check workload credentials (on GCE, GKE, GAE and Serverless)**
  1. If true,
    ...
    1. If only one of the **"workload_identity_provider"** and
       **"service_account_email"** fields is present in the
       **"~/.config/gcloud/certificate_config.json"** [configuration file]][9]
       return an error identifying that _[END]_
