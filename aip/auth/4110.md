...
identifies either a human user or an impersonated service account that needs
to authenticate to access Google APIs. The auth libraries **must** support 
the human user credential type and **may** support the impersonated service
...
gCloud uses to store the default credential JSON. The output is the access
...
- **2021-03-29**: Add impersonated service account to gcloud credenetails.
