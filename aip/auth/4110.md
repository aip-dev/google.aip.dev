...
1. **Determine auth flows**
  1. If the credential is a [gcloud credential JSON](5) go to step (5.3)
  1. If the credential is a [service account key JSON](6):
     1. If neither `scope` or `audience` is available, return an error _[END]_
     1. If `audiences` are **explicitly** provided by developer, which indicates that the developer asks to use self-signed JWT, go to step (5.1)
     1. If `audiences` are **not explicitly** provided:
        1. If scope is available and [scope is **not supported** for self-signed JWT](8), go to step (5.2)
        1. Otherwise, go to step (5.1)
1. **Execute auth flows**
  ...
  1. Use OAuth 2.0 flow to exchange for an access token _[END]_
     1. If `target audience` is provided by the developer, get an [identity token](7) _[END]_
