...
$.when($.ready).then(() => {
  $('.language-graphviz').each((_, el) => {
    ...
    let graphviz = $(el).find('code').text();

    // Render a SVG and replace the original graphviz code with it.
    let viz = new Viz();
    viz.renderSVGElement(graphviz).then((svg) => {
      $('<p class="digraph"></p>').append(svg).insertBefore(pre);
      pre.remove();
    });
  });
});
